...
import { useNavigate } from "react-router-dom";
import { api } from "../../services/api";
import { CategoryButton, Container, ContainerItems, Title, SubTitle } from "./styles";
...
export function CategoryCarousel() {
    ...
    return (
        <Container>
            <Title>Categorias</Title>
            <SubTitle>Escolha o que você quer hoje</SubTitle>
            <Carousel responsive={responsive} infinite={true} itemClass="carousel-item">
                {categories.map((category) => (
                    <ContainerItems key={category.id} imageUrl={category.url}>
                        <CategoryButton
                            onClick={() => navigate({ pathname: "/cardapio", search: `?categoria=${category.id}` })}
                        >
                            {category.name}
                        </CategoryButton>
                    </ContainerItems>
