...
import {
  Container, VideoBackground, Overlay, Topbar, TopbarBrand, TopbarRight,
  StatusDot, Panel, PanelAccent, LogoWrapper, LogoName, LogoSub,
  Divider, DividerLine, DividerText, Form, Field, FieldLabel, FieldInner,
  FieldInput, Tick, ForgotLink, RegisterText, Tracker, TrackerTitle,
  Step, StepIcon, StepInfo, StepName, StepDesc, StepBadge,
  OrderStatus, StatusIndicator, StatusText, Ticker, TickerLabel,
  TickerTrack, TickerItem, SuccessOverlay, SuccessEmoji, SuccessTitle,
  SuccessSub, LoginButton,
} from './styles';
...
const STEPS = [
  { icon: '🍞', name: 'Pão de baixo',  desc: 'aguardando email...' },
  { icon: '🥬', name: 'Alface',         desc: 'aguardando senha...' },
  { icon: '🥩', name: 'Hambúrguer',     desc: 'aguardando senha...' },
  { icon: '🧀', name: 'Queijo',         desc: 'aguardando senha...' },
  { icon: '🍅', name: 'Tomate',         desc: 'aguardando senha...' },
  { icon: '🍞', name: 'Pão de cima',    desc: 'feche ao entrar!'    },
];
...
export function Login() {
  ...
  useEffect(() => { setEmailFilled(/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(emailValue)); }, [emailValue]);
  useEffect(() => { setSenhaFilled(passwordValue?.length >= 6); }, [passwordValue]);

  const getStepDone = (i) => i === 0 ? emailFilled : i <= 4 ? senhaFilled : false;
  const getStepDesc = (step, i) => getStepDone(i) ? '✓ adicionado!' : step.desc;
    ...
    <Container>
      ...
      <main style={{
        ...
        <Panel>
          ...
          <Divider>
            <DividerLine /><DividerText>acesso</DividerText><DividerLine />
          </Divider>
          <Form onSubmit={handleSubmit(onSubmit)}>
            <Field filled={emailFilled}>
              <FieldLabel filled={emailFilled}>🍞 Email — pão de baixo</FieldLabel>
              <FieldInner>
                <FieldInput type="email" placeholder="seu@email.com" filled={emailFilled} hasError={!!errors.email} {...register("email")} autoComplete="email" />
                <Tick visible={emailFilled}>✓</Tick>
              </FieldInner>
              {errors.email && <p style={{ color: '#FF6B6B', fontSize: 11, marginTop: 6 }}>{errors.email.message}</p>}
            </Field>
            <Field filled={senhaFilled}>
              <FieldLabel filled={senhaFilled}>🥩 Senha — recheio secreto</FieldLabel>
              <FieldInner>
                <FieldInput type="password" placeholder="Mín. 6 caracteres" filled={senhaFilled} hasError={!!errors.password} {...register("password")} autoComplete="current-password" />
                <Tick visible={senhaFilled}>✓</Tick>
              </FieldInner>
              {errors.password && <p style={{ color: '#FF6B6B', fontSize: 11, marginTop: 6 }}>{errors.password.message}</p>}
            </Field>
            <ForgotLink href="#">Esqueceu a senha?</ForgotLink>
            <LoginButton type="submit" disabled={!isReady}>MONTAR &amp; ENTRAR</LoginButton>
          </Form>
          <RegisterText>
            Primeira vez aqui?{' '}
            <a href="/cadastro" style={{ color: 'rgba(255,248,240,.45)', borderBottom: '1px solid rgba(255,248,240,.12)', textDecoration: 'none', paddingBottom: 1 }}>
