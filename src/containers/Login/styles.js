...
const burnGlow = keyframes`
  0%, 100% { box-shadow: 0 0 40px rgba(255, 100, 0, 0.15), 0 0 80px rgba(255, 60, 0, 0.08); }
  50%       { box-shadow: 0 0 60px rgba(255, 100, 0, 0.25), 0 0 120px rgba(255, 60, 0, 0.12); }
`;
...
export const LeftContainer = styled.div`
  ...
  &::before {
    ...
    background: linear-gradient(
      ...
      rgba(12, 10, 7, 0.55) 0%,
      rgba(12, 10, 7, 0.2) 50%,
      rgba(12, 10, 7, 0.7) 100%
    );
    z-index: 1;
  }

  /* scanline — efeito tela quente */
  &::after {
    content: '';
    position: absolute;
    left: 0; right: 0;
  ...
  img {
    ...
    filter:
      drop-shadow(0 8px 32px rgba(255, 80, 0, 0.5))
      drop-shadow(0 0 64px rgba(255, 40, 0, 0.2));
  }
...
export const RightContainer = styled.div`
  ...
  &::after {
    ...
    border-radius: 50%;
    animation: ${burnGlow} 5s ease-in-out infinite;
    pointer-events: none;
  }
...
export const Title = styled.h2`
  ...
  text-align: center;
  margin-bottom: 2.25rem;
  animation: ${flicker} 8s ease-in-out infinite, ${fadeUp} 0.6s 0.2s both;

  span {
    font-family: "Road Rage", sans-serif;
    background: linear-gradient(90deg, #ff6b1a, #ffaa00);
    -webkit-background-clip: text;
    -webkit-text-fill-color: transparent;
    background-clip: text;
...
export const InputContainer = styled.div`
  ...
  input {
    width: 100%;
    height: 52px;
    border: 1.5px solid rgba(255, 160, 60, 0.15);
    border-radius: 8px;
    padding: 0 16px;
    ...
    &::placeholder {
      color: rgba(255, 230, 180, 0.18);
    }

    &:-webkit-autofill,
    &:-webkit-autofill:hover,
    &:-webkit-autofill:focus {
      -webkit-box-shadow: 0 0 0 1000px #1a1208 inset;
      -webkit-text-fill-color: #fdf0dc;
      transition: background-color 9999s;
    }
