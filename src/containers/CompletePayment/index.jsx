...
const ActionButton = styled.a`
  ...
  letter-spacing: 5px;
  color: #fff;
  box-shadow: 0 4px 0 #5A0005, 0 8px 30px rgba(200,0,10,.35);
  transition: all .22s ease;

  &:hover {
    transform: translateY(-2px);
    box-shadow: 0 6px 0 #5A0005, 0 14px 40px rgba(200,0,10,.55);
  }
`;
...
const STATUS_MAP = {
  succeeded: {
    ...
    accentColor: "#22C55E",
    icon: (
      <svg width="28" height="28" viewBox="0 0 16 14" fill="none">
        <path fillRule="evenodd" clipRule="evenodd" d="M15.4695 0.232963C15.8241 0.561287 15.8454 1.1149 15.5171 1.46949L6.14206 11.5945C5.97228 11.7778 5.73221 11.8799 5.48237 11.8748C5.23253 11.8698 4.99677 11.7582 4.83452 11.5681L0.459523 6.44311C0.145767 6.07557 0.18937 5.52327 0.556912 5.20951C0.924454 4.89575 1.47676 4.93936 1.79051 5.3069L5.52658 9.68343L14.233 0.280522C14.5613 -0.0740672 15.1149 -0.0953599 15.4695 0.232963Z" fill="white"/>
      </svg>
    ),
    primaryUrl: "/",
    primaryText: "VOLTAR À LOJA",
    secondaryUrl: null,
  },
  processing: {
    text: "Pagamento em Processamento...",
...
export function CompletePayment() {
  ...
  const current = STATUS_MAP[status] || STATUS_MAP.default;

  return (
    <Wrapper>
      <Card>
        <TopAccent $color={current.accentColor} />
        <LogoImg src={Logo} alt="Dev Burguer" />

        <IconCircle $color={current.color}>
          {current.icon}
        </IconCircle>

        <StatusText>{current.text}</StatusText>
          ...
          <>
            ...
            <DetailsTable>
              <DetailRow>
                ...
                <DetailValue>{intentId}</DetailValue>
              </DetailRow>
              <DetailRow>
        ...
        <ActionButton href={current.primaryUrl}>{current.primaryText}</ActionButton>
        {current.secondaryUrl && (
          <SecondaryButton href={current.secondaryUrl}>{current.secondaryText}</SecondaryButton>
        )}
      </Card>
    </Wrapper>
  );
